...
namespace NAssist {
	...
	void GSoVITSAssist::init()
	{
		...
		//set the sovits model and gpt model
		{
			...
			std::shared_ptr<HttpRequest> request = HttpRequest::CreateRequest(request_host, set_gpt_weights_endpoint, HttpRequestMethod::get);
			request->AddHeader("weights_path", m_GSoVITSModel.gpt_weights);
			std::cout << request->Receive() << std::endl;
		}
		{
			
			std::shared_ptr<HttpRequest> request = HttpRequest::CreateRequest(request_host, set_sovits_weights_endpoint, HttpRequestMethod::get);
		...
		m_thread = std::thread([this]() {
			while (!m_stoped)
			{
				...
				m_condition.wait(lock, [this] { return !m_msg_queue.empty(); }); // 等待条件
	...
	void GSoVITSAssist::commitRequest2GSoVits(const std::string& msg) {
		...
		if (result.index() == 0)
		{
			...
			{
				std::shared_ptr<HttpRequest> request = HttpRequest::CreateRequest(request_host, inference_endpoint, HttpRequestMethod::post);
