...
PeriodicTask::PeriodicTask(std::function<void()> task, int interval)
    : m_task(task), m_interval(interval), m_isRunning(false), m_io_ctx(),
...
void PeriodicTask::Start() {
  ...
  m_timer.async_wait(boost::beast::bind_front_handler(&PeriodicTask::onTimerEnd,
                                                      shared_from_this()));
...
void PeriodicTask::CommitTask() { m_io_ctx.run(); }
...
void PeriodicTask::onTimerEnd(const boost::system::error_code &e) {
  ...
  m_timer.expires_at(m_timer.expiry() +
