...
void LLKLogger::addLogger(std::shared_ptr<spdlog::logger> logger,
                          LLKLogger::Type type) {
...
std::shared_ptr<spdlog::logger> LLKLogger::GetLogger(LLKLogger::Type type) {
  if (LLKLogger::instance()->m_loggers.contains(type))
    return LLKLogger::instance()->m_loggers.at(type);
